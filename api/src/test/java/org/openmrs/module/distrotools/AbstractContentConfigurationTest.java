...
import java.lang.ClassLoader;import java.lang.Exception;import static org.hamcrest.Matchers.*;
...
public class AbstractContentConfigurationTest {
	...
	public void getClassLoader_shouldGetModuleLoaderIfModuleSpecified() throws Exception {
		...
		Mockito.when(ModuleFactory.getModuleClassLoader("distrotools")).thenReturn(testClassLoader);

		// Test with a moduleId - should return the module class loader
		TestContentConfiguration config = new TestContentConfiguration();
		config.setModuleId("distrotools");
		Assert.assertThat(config.getModuleId(), is("distrotools"));
