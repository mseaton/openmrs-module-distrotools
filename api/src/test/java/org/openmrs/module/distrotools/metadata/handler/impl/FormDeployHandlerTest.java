...
public class FormDeployHandlerTest extends BaseModuleContextSensitiveTest {
	...
	public void integration() {
		...
		distroToolsService.installObject(encounterType("Other Encounter", "Testing", "enc-type2-uuid"));

		// Check installing new
		distroToolsService.installObject(form("New name", "New desc", "enc-type1-uuid", "1.0", "form-uuid"));
		...
		distroToolsService.installObject(form("Updated name", "Updated desc", "enc-type2-uuid", "2.0", "form-uuid"));
		...
		distroToolsService.uninstallObject(distroToolsService.fetchObject(Form.class, "form-uuid"), "Testing");

		Assert.assertThat(Context.getFormService().getFormByUuid("form-uuid").isRetired(), is(true));

		// Check re-install unretires
		distroToolsService.installObject(form("Unretired name", "Unretired desc", "enc-type2-uuid", "2.0", "form-uuid"));
