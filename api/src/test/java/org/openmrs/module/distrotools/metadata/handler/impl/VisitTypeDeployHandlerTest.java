...
import static org.openmrs.module.distrotools.metadata.bundle.CoreConstructors.visitType;
...
public class VisitTypeDeployHandlerTest extends BaseModuleContextSensitiveTest {
	...
	public void integration() {
		...
		distroToolsService.installObject(visitType("New name", "New desc", "obj-uuid"));
		...
		distroToolsService.installObject(visitType("Updated name", "Updated desc", "obj-uuid"));
		...
		distroToolsService.uninstallObject(distroToolsService.fetchObject(VisitType.class, "obj-uuid"), "Testing");

		Assert.assertThat(Context.getVisitService().getVisitTypeByUuid("obj-uuid").isRetired(), is(true));

		// Check re-install unretires
		distroToolsService.installObject(visitType("Unretired name", "Unretired desc", "obj-uuid"));
